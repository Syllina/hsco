...
parser = (ModListFile <$> strOption (long "outdir" <> short 'o' <> metavar "DIRECTORY" <> help "the output directory")
                  <*> strOption (long "file" <> short 'f' <> metavar "FILE" <> help "the mod list file"))
     <|> (CreateList <$> strOption (long "name-list" <> metavar "FILE" <> help "the name list to create a mod list from")
                     <*> strOption (long "output" <> short 'o' <> metavar "FILE" <> help "the mod list file")
...
deal1 :: FilePath -> FilePath -> IO ()
deal1 dir file = do
    res <- loadModList file
    ...
    when (inp == "y") $ do
        saveModList file newList
        procActions (dir </> "") actions
...
main = do
    ...
    case options of
        ModListFile dir file  -> do
            deal1 (T.unpack dir) (T.unpack file)
        CreateList file output gameVer -> do
            res <- eitherDecodeFileStrict $ T.unpack file
            let nameList = case res of
                    Left err -> error $ T.pack err
                    Right list -> list :: [Text]
            let modList = ModList {
                gameVersion = gameVer,
                modItems = generateModrinthManuals nameList
            }

            saveModList (T.unpack output) modList
